...
function handlePhoto1Change(e: Event) { handleFileChange(e, 'photo1', preview1) }
function handlePhoto2Change(e: Event) { handleFileChange(e, 'photo2', preview2) }
function handlePhoto3Change(e: Event) { handleFileChange(e, 'photo3', preview3) }

// Terms modal actions
function openTermsModal() { showTermsModal.value = true }
function acceptTerms() { agreedToTerms.value = true; showTermsModal.value = false }
...
<template>
  ...
  <div class="w-full bg-white flex flex-col sm:flex-row items-center sm:justify-between gap-2 px-4 sm:px-8 py-2 border-b border-gray-200 text-xs sm:text-sm font-semibold text-center sm:text-left">
    ...
    <div class="flex flex-col sm:flex-row gap-1 sm:gap-6 items-center text-[#002B5C]">
      <div class="flex items-center gap-2"><span class="hidden sm:inline">📞</span> <span class="font-bold">Call Us</span> <span class="font-normal">+639927594673/09927594673</span></div>
  ...
  <div class="flex-grow flex items-center justify-center bg-[#F8FAFC] min-h-screen">
    ...
    <!-- Terms Modal -->
    <div v-if="showTermsModal" class="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 px-4">
      <div class="bg-white w-full max-w-lg rounded-t-2xl sm:rounded-2xl shadow-xl p-4 sm:p-6 relative overflow-y-auto max-h-[80vh]">
        ...
        <div class="mt-6 flex justify-end gap-3">
          <button @click="showTermsModal = false" class="px-4 py-2 bg-gray-300 text-gray-800 rounded-lg font-semibold hover:bg-gray-400">Cancel</button>
          <button @click="acceptTerms" class="px-5 py-2 bg-[#FF2D2D] text-white font-semibold rounded-lg hover:bg-[#E02626]">I Agree</button>
        </div>
      </div>
    </div>
  </div>
