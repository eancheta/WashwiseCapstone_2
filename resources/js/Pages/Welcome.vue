...
<template>
  ...
  <nav class="w-full bg-[#182235] flex items-center px-8 py-2 text-white font-semibold shadow z-10">
    ...
    <div class="flex items-center gap-4 ml-8">
      <button @click="openLoginModal" class="text-white font-semibold hover:text-[#FF2D2D] transition" style="font-size:1rem; line-height:1; background:none; border:none; cursor:pointer;">
        Log in
      </button>
      <button @click="openRegisterModal" class="px-6 py-2 rounded-full border-2 font-semibold transition" style="border-color:#FF2D2D; color:#FF2D2D; font-size:1rem; line-height:1; background:none; cursor:pointer;">
        Register
      </button>
    </div>
  </nav>
  ...
  <div v-if="showLoginModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg p-8 w-full max-w-xs shadow-lg relative">
      ...
      <div class="flex flex-col gap-4">
        <Link href="/login" class="px-4 py-2 rounded bg-[#182235] text-white font-semibold text-center hover:bg-[#FF2D2D] transition">User</Link>
  ...
  <div v-if="showRegisterModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg p-8 w-full max-w-xs shadow-lg relative">
      ...
      <div class="flex flex-col gap-4">
        <Link href="/register" class="px-4 py-2 rounded bg-[#182235] text-white font-semibold text-center hover:bg-[#FF2D2D] transition">User</Link>
  ...
  >
    ...
    <div class="relative z-10 flex flex-col items-center justify-center w-full px-4 py-20">
      ...
      <Link href="#" class="px-8 py-3 rounded-full bg-[#FF2D2D] text-white font-bold text-lg shadow hover:bg-[#d72626] transition">
        Explore More
      </Link>
    </div>
  </section>
